...
from prediction_foot import charger_donnees, calculer_features, FEATURES

ANNEES_TEST = range(2014, 2026)   # on teste chaque saison de 2014 à 2025

def backtest():
    df = charger_donnees()
    df, _, _ = calculer_features(df)   # features causales calculées une seule fois
    ...
    for annee in ANNEES_TEST:
        ...
        modele.fit(train[FEATURES], train["resultat"])
        pred = modele.predict(test[FEATURES])
