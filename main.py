...
from prediction_foot import (charger_donnees, calculer_features, entrainer,
                             proba_match as _proba_match)
from prediction_score import entrainer_score, predire_score
...
def preparer(mode):
    ...
    modele = entrainer(df, mode=mode, rapport=False)
    mh, ma = entrainer_score(df)          # modèle de score (double Poisson)
    return modele, elos, hist, etat, mh, ma, df["date"].max().date()


def proba_match(modele, etat, hist, dom, ext, neutre, mode):
    f_dom = np.mean(hist[dom]) if len(hist[dom]) else 1.0
    f_ext = np.mean(hist[ext]) if len(hist[ext]) else 1.0
    p = _proba_match(modele, etat, dom, ext, neutre, mode=mode)
    return p, f_dom, f_ext


# Sélecteur de mode dans la barre latérale
MODE_LABELS = {
    "equilibre": "Équilibré (prédit les nuls)",
    "precision": "Précision max (modèle d'origine)",
}
with st.sidebar:
    st.header("Réglage du modèle")
    mode = st.radio("Mode", list(MODE_LABELS), format_func=MODE_LABELS.get, index=0)
    st.caption(
        "**Équilibré** : pondère les classes pour vraiment prédire les nuls "
        "(recall ~37 %, macro-F1 ~0,52), précision globale ~55 %.\n\n"
        "**Précision max** : maximise la précision affichée (~60 %) mais ne "
        "prédit quasiment jamais les nuls."
    )

modele, elos, hist, etat, mh, ma, derniere_date = preparer(mode)
equipes = sorted(elos)
...
p, f_dom, f_ext = proba_match(modele, etat, hist, domicile, exterieur, neutre, mode)
...
sc = predire_score(mh, ma, etat, domicile, exterieur, neutre)
i, j = sc["score_exact"]
...
with sa:
    ...
    <div class="vs-card">
      ...
      <div style="font-size:46px; font-weight:800; color:#FFFFFF; margin:4px 0;">
        {i} <span style="color:#21BF73;">–</span> {j}
      </div>
      <div class="vs-elo">{domicile} · {exterieur}</div>
      <div class="vs-elo" style="margin-top:10px;">
        Buts attendus : {sc['lambda_dom']:.2f} vs {sc['lambda_ext']:.2f}
      </div>
    </div>
    """, unsafe_allow_html=True)

    (a, b), pa = sc["score_si_domicile"]
    (c, d), pn = sc["score_si_nul"]
    (e, f), pe = sc["score_si_exterieur"]
    st.markdown(
        f"<p class='note'>Score le plus probable selon l'issue :<br>"
        f"• {labels['domicile']} : <b>{a}-{b}</b> ({pa:.0%})<br>"
        f"• Match nul : <b>{c}-{d}</b> ({pn:.0%})<br>"
        f"• {labels['exterieur']} : <b>{e}-{f}</b> ({pe:.0%})</p>",
        unsafe_allow_html=True)

with sb:
    # Heatmap de la matrice des scores (0..5 buts pour rester lisible)
    NB = 6
    M = sc["matrice"][:NB, :NB]
    z = (M * 100).round(1)
    fig_m = go.Figure(data=go.Heatmap(
        z=z, x=[f"{k}" for k in range(NB)], y=[f"{k}" for k in range(NB)],
        colorscale="Greens", showscale=False,
        text=z, texttemplate="%{text:.1f}%", textfont={"size": 11},
...
st.markdown('<p class="note">Le score est estimé par deux régressions de Poisson '
            '(buts attendus de chaque équipe), d\'où se déduit la probabilité de chaque '
            'score exact. Les matchs nuls (1-1, 0-0…) ressortent naturellement quand les '
            'deux équipes sont proches.</p>', unsafe_allow_html=True)
